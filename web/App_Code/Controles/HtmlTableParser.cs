...
namespace RDev.Correios.Helpers
{
    public class HtmlTableParser
    {
            ...
            foreach (Match rowMatch in rowMatches)
            {
                ...
                if (!rowMatch.Value.Contains("<th"))
                {
                    ...
                    for (int columnIndex = 0; columnIndex < cellMatches.Count; columnIndex++)
                    {
                        ...
                        string name = Regex.Replace(cellMatches[columnIndex].Groups[1].ToString(), pattern, "");
                        dataRow[columnIndex] = name;
                    }
        ...
        private static DataColumn[] ParseColumns(string tableHtml)
        {
            ...
            {
                const string pattern = (@"<[^>]+>");
                string columnName = Regex.Replace(headerMatch.Groups[1].ToString(), pattern, "");

                list.Add(new DataColumn(columnName));
            }
            return list.ToArray();
        }
