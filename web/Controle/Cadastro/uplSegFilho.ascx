<%@ Control Language="C#" AutoEventWireup="true" CodeFile="uplSegFilho.ascx.cs" Inherits="controle_uplLogo" %>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("#mulitplefileuploader").uploadFile({
            url: '<%= MetodosFE.BaseURL%>/Controle/Cadastro/UploadSegFilho.ashx',
            autoSubmit: true,
            multiple: true,
            dragDrop: true,
            showDone: false,
        ...
        $('#btnSalvarOrdem').click(function () {
            ...
            $.ajax({
                type: 'POST'
    , url: "<%=MetodosFE.BaseURL %>/Webservices/ControleImagensSegFilho.asmx/salvarOrdem"
    ...
    function excluirImagem(idFoto) {
        $.ajax({
            type: 'POST'
                , url: "<%=MetodosFE.BaseURL %>/Webservices/ControleImagensSegFilho.asmx/excluirImagemDados"
                , contentType: 'application/json; charset=utf-8'
                , dataType: 'json'
    ...
    function carregaImagens() {
            ...
            $.ajax({
                type: 'POST'
                , url: "<%=MetodosFE.BaseURL %>/Webservices/ControleImagensSegFilho.asmx/getImagensDados"
                , contentType: 'application/json; charset=utf-8'
                , dataType: 'json'
    ...
    <div>
        <ul id="listaImagens" >
            <asp:Repeater runat="server" ID="repImagens">
                <ItemTemplate>
                    <li id="<%# ((Modelos.ImagemSegFilhoVO)Container.DataItem).Id %>">
                        <img src=" <%# String.Format("{0}/ImagensLQ/{1}",MetodosFE.BaseURL, ((Modelos.ImagemSegFilhoVO)Container.DataItem).Nome) %>" > 
                        <img src="<%= String.Format("{0}/Controle/comum/img/close-button.png",MetodosFE.BaseURL) %>" class="botaoExcluirImagem" rel="<%# ((Modelos.ImagemSegFilhoVO)Container.DataItem).Id %>" >
                            </li>
                </ItemTemplate>
            </asp:Repeater>
        </ul>
