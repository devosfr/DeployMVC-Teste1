<%@ Control Language="C#" AutoEventWireup="true" CodeFile="uplCategoria.ascx.cs" Inherits="controle_uplLogo" %>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("#mulitplefileuploader").uploadFile({
            url: '<%= MetodosFE.BaseURL%>/Controle/Cadastro/UploadCategoria.ashx',
            autoSubmit: true,
            multiple: true,
            dragDrop: true,
            showDone: false,
        ...
        $('#btnSalvarOrdem').click(function () {
            ...
            $.ajax({
                type: 'POST'
    , url: "<%=MetodosFE.BaseURL %>/Webservices/ControleImagensCategorias.asmx/salvarOrdem"
    ...
    function excluirImagem(idFoto) {
        $.ajax({
            type: 'POST'
                , url: "<%=MetodosFE.BaseURL %>/Webservices/ControleImagensCategorias.asmx/excluirImagemDados"
                , contentType: 'application/json; charset=utf-8'
                , dataType: 'json'
    ...
    function carregaImagens() {
            ...
            $.ajax({
                type: 'POST'
                , url: "<%=MetodosFE.BaseURL %>/Webservices/ControleImagensCategorias.asmx/getImagensDados"
                , contentType: 'application/json; charset=utf-8'
                , dataType: 'json'
    ...
    <div>
        <ul id="listaImagens" >
            <asp:Repeater runat="server" ID="repImagens">
                <ItemTemplate>
                    <li id="<%# ((Modelos.ImagemCategoriaVO)Container.DataItem).Id %>">
                        <img src=" <%# String.Format("{0}/ImagensLQ/{1}",MetodosFE.BaseURL, ((Modelos.ImagemCategoriaVO)Container.DataItem).Nome) %>" > 
                        <img src="<%= String.Format("{0}/Controle/comum/img/close-button.png",MetodosFE.BaseURL) %>" class="botaoExcluirImagem" rel="<%# ((Modelos.ImagemCategoriaVO)Container.DataItem).Id %>" >
                            </li>
                </ItemTemplate>
            </asp:Repeater>
        </ul>
