<%@ Control Language="C#" AutoEventWireup="true" CodeFile="uplImagemProduto.OLD.ascx.cs" Inherits="controle_uplLogo" %>
...
<script type="text/javascript">
    $(document).ready(function () {
        $("#mulitplefileuploader").uploadFile({
            url: '<%= MetodosFE.BaseURL%>/Controle/Cadastro/UploadImagemProduto.ashx',
            autoSubmit: true,
            multiple: true,
            dragDrop: true,
            showDone: false,
    ...
    <div>
        <ul id="listaImagens" >
            <asp:Repeater runat="server" ID="repImagens">
                <ItemTemplate>
                    <li id="<%# ((Modelos.ImagemProduto)Container.DataItem).Id %>">
                        <img src=" <%# String.Format("{0}/ImagensLQ/{1}",MetodosFE.BaseURL, ((Modelos.ImagemProduto)Container.DataItem).Nome) %>" > 
                        <img src="<%= String.Format("{0}/Controle/comum/img/close-button.png",MetodosFE.BaseURL) %>" class="botaoExcluirImagem" rel="<%# ((Modelos.ImagemProduto)Container.DataItem).Id %>" >
                            </li>
                </ItemTemplate>
            </asp:Repeater>
        </ul>
