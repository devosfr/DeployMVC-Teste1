...
public class Upload : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    private Repository<ImagemSegPaiVO> repoFotoDado
    {
        get
        {
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        try
        {
            ...
            IList<ImagemSegPaiVO> colecaofotos = repoFotoDado.All().Where(x => x.segPai.Id == idProduto).ToList();
    ...
    public void upload(int idCodigo, string tmpFoto, string nome, int contagem = 1)
    {
        ...
        if (idCodigo != 0)
        {
            ...
            SegmentoPaiVO segPai = repoSegmentoPai.FindBy(idProduto);
            
            FileInfo arquivo = new FileInfo(tmpFoto);

            ImagemSegPaiVO foto = new ImagemSegPaiVO();
            
            foto.segPai = segPai;
            foto.Ordem = 0;
            repoFotoDado.Add(foto);
            foto.Nome = idProduto + "-" + segPai.chave + "-" + foto.Id + arquivo.Extension;
