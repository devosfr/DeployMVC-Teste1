...
public partial class Controle_Cadastro_Estado : System.Web.UI.Page
{
    ...
    protected void Carregar()
    {
        try
        {
            Tela tela = repoTela.FindBy(x => x.nomeFixo == "Produtos");

            if (tela == null)
            {

                tela = new Tela() { nome = "Produtos", nomeFixo = "Produtos", pagina = null };
                repoTela.Add(tela);
            }
    ...
    protected void btnAlterar_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (chkTelaVisivel.Checked)
            {
                ...
                if (pagina == null)
                {
                    pagina = new PaginaDeControleVO() { nome = tela.nome, pagina = MetodosFE.BaseURL + "/Controle/Cadastro/Produto/Produtos.aspx",fixa=true };
