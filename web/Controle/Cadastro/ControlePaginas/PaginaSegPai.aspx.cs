...
public partial class Controle_Cadastro_Estado : System.Web.UI.Page
{
    ...
    {
        nome = "Tela de Seg. Pai";
        nome2 = "Tela de Seg. Pai";
    ...
    protected void Carregar()
    {
        try
        {
            Tela tela = repoTela.FindBy(x => x.nomeFixo == "SegmentoPai");

            if (tela == null)
            {

                tela = new Tela() { nome = "Segmento Pai", nomeFixo = "SegmentoPai", pagina = null };
                repoTela.Add(tela);
            }
    ...
    protected void btnAlterar_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (chkTelaVisivel.Checked == true)
            {
                pagina = repoPaginasControle.FindBy(x => x.nome == tela.nome);

                if (pagina == null)
                {
                    pagina = new PaginaDeControleVO() { nome = tela.nome, pagina = MetodosFE.BaseURL + "/Controle/Cadastro/SegmentosPai.aspx", fixa = true };
