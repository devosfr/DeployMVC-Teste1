...
ControleLogin.statusLoginGerenciador();
    ...
    public void upload(int idCodigo, string tmpFoto, string nome, int contagem = 1)
    {
        ...
        if (idCodigo != 0)
        {
            ...
            Destaque destaque = RepositorioDestaque.FindBy(idCodigo);

            FileInfo arquivo = new FileInfo(tmpFoto);


            string imagemAntiga = null;
            if (!String.IsNullOrEmpty(destaque.Imagem))
                imagemAntiga = destaque.Imagem;

            destaque.Imagem = idProduto + "-destaque-" + destaque.Nome.ToSeoUrl() + "-" + destaque.Id + arquivo.Extension;


            //Processo de upload
            uplImage.Resize(tmpFoto, uplImage.diretorioHQ + "\\" + destaque.Imagem, objUpload.TamFotoGrW, objUpload.TamFotoGrH, objUpload.Qualidade, objUpload.Cor, objUpload.Configuracao);
            uplImage.Resize(tmpFoto, uplImage.diretorioLQ + "\\" + destaque.Imagem, objUpload.TamFotoPqW, objUpload.TamFotoPqH, objUpload.Qualidade, objUpload.Cor, objUpload.Configuracao);
